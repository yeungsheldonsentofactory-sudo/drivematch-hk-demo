...
import {
  ...
  FileText, Heart, Menu, MessageCircle, Phone, Plus, Search, Send,
...
function BuyerHome({ onScreen }) {
  ...
  const send = (event) => {
    ...
    const next = [...chat, { from: 'buyer', body: message.trim() }]
    setChat(next); localStorage.setItem('apex-chat', JSON.stringify(next)); setMessage('')
    window.setTimeout(() => {
      const reply = [...next, { from: 'agent', body: '已收到，你的專屬顧問會盡快回覆。' }]
      setChat(reply); localStorage.setItem('apex-chat', JSON.stringify(reply))
    }, 450)
  }
  ...
  </main><VehicleDetailDialog car={activeCar} onClose={() => setActiveCar(null)} onChat={() => { setActiveCar(null); setChatOpen(true) }}/><ChatWidget open={chatOpen} setOpen={setChatOpen} chat={chat} message={message} setMessage={setMessage} send={send}/></div>
}
...
function SellerPage({ onScreen }) {
  ...
  const submit = (event) => { event.preventDefault(); setTouched(Object.fromEntries(Object.keys(errors).map((key) => [key, true]))); if (Object.values(errors).some(Boolean)) return; setSubmitted(true) }
